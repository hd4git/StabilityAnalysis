...
$src="/data/borth/hdhiman/janssen/integration_sites/filter_regions/src2/";
$results="/data/borth/hdhiman/janssen/integration_sites/filter_regions/results2";
...
open(gff, "$src/GCF_003668045.1_CriGri-PICR_genomic_parsed.gff") or die "Can't open gff";
...
###################################################################################################
$file_in=$ARGV[1]; 
$file_out="filter_regions_v1_".$ARGV[1].".txt";

open(INDEX, "<$src/ref_split/$file_in") or die "Can't open INDEX";
...
foreach(<INDEX>)
{
    ...
    for($win=0; $win<=$index[1]; $win+=2000)
    {
	$start=$win+1;
	$stop=$win+2000;
	$pos=$win+1000;
